...
names(damage_full) <- c("site_name", "seed_zone", "site_code", "big_region", "dec_latitude", "dec_longitude", "site_area_ha",
								 "gsl", "growing_deg_days_c", "feb_mean_temp_c", "jul_mean_temp_c", "loc", "family", "individual", 
								 "field_code", "x_coord", "y_coord", "xy_coord", "block", "prev_damage", "curr_damage",
								 "total_damage")
```
...
Boxplots of lesions by Cavers region:
...
```{r}
cor_dam <- cor(damage_full$prev_damage, damage_full$curr_damage, method = "pearson")

damage_nozero_dam <- damage_full %>%
	filter(prev_damage != 0,
				 curr_damage != 0)

cor_dam_nozero <- cor(damage_nozero_dam$prev_damage, damage_nozero_dam$curr_damage, method = "pearson")

cor_dam

cor_dam_nozero
```
Thee isn't much of a correlation between the two
...
Family level groupings didn't appear to predict weevil damage at all and population (site) level groupings don't predict very much, so what if I group into larger groups such as `seed_zone` and `big_region`?
...
__Think about using family as a random effect in the models, rather than a fixed effect__
