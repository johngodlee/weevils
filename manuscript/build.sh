...
# Run analyses
Rscript ../analysis/analyses_plots.R
Rscript ../analysis/analyses_stats_final_final.R
...
sed -i '10s/.*/{Cor. Struct.} \& {AIC} \& {logLik} \& {r2c} \\\\/' include/gls_aic_comp.tex

sed -i '10s/.*/{Fixed eff.} \& {Random eff.} \& {AIC} \& {logLik} \& {r2c} \& {r2m} \\\\/' include/binom_lmer_aic_comp.tex

sed -i '10s/.*/{Fixed eff.} \& {Random eff.} \& {AIC} \& {logLik} \& {r2c} \& {r2m} \\\\/' include/lmer_aic_comp.tex
